// Mock 数据（与 packages/website/lib/mock.ts 对齐）
// 当前阶段：service 直接返回 mock，shape 与 admin FastAPI 一致
// TODO：接 Cloudflare D1 / Workers AI 时，把这些函数替换成真实查询
...
function buildScoreDistribution(win: number, draw: number, loss: number): ScoreDistributionItem[] {
...
const PREDICTION_DATE = "2026-06-12T01:00:00Z";
...
function makePredictionFor(
  ...
  return {
    ...
    key_factors: [
      { factor: "近期主场战绩 5 战 4 胜 1 平，状态火热", impact: "positive_home" as const, weight: 0.32 },
      { factor: "主力前锋伤愈回归，进攻端威胁显著提升", impact: "positive_home" as const, weight: 0.24 },
      { factor: "客场连续 3 场失球超过 1.5 个", impact: "positive_home" as const, weight: 0.18 },
      { factor: "近 5 场 H2H 客队 2 胜 2 平 1 负，存在心理优势", impact: "positive_away" as const, weight: 0.16 },
      { factor: "天气：高温 33°C，可能影响主队体能", impact: "neutral" as const, weight: 0.10 },
    ],
    key_players: [
      { team: "home" as const, name: "Vinícius Jr.", position: "Forward", strength: 9.1, form_score: 8.7, rationale: "近 5 场贡献 4 球 2 助攻，盘带突破极具威胁", source: "mixed" as const },
      { team: "home" as const, name: "Casemiro", position: "Midfielder", strength: 8.6, form_score: 8.2, rationale: "中场拦截与出球枢纽，防守贡献稳定", source: "roster" as const },
      { team: "away" as const, name: "Achraf Hakimi", position: "Defence", strength: 8.8, form_score: 8.0, rationale: "右路攻防俱佳，速度与助攻能力突出", source: "news" as const },
    ],
...
export function getMatches(status?: string): MatchListResponse {
  const list = status ? MATCHES.filter((m) => m.status === status) : MATCHES;
  return { matches: list, total: list.length };
}

export function getMatchDetail(id: string): MatchDetailResponse | null {
  ...
  return {
    ...
    home_form: {
      ...
      as_of: "2026-06-10",
      last5_w: 4, last5_d: 1, last5_l: 0,
      goals_for_avg: 2.4, goals_against_avg: 0.6,
    },
    away_form: {
      team_id: m.away_team_id,
      as_of: "2026-06-10",
      last5_w: 2, last5_d: 1, last5_l: 2,
...
export function getPredictionBundle(matchId: string): PredictionBundle | null {
