...
- 风险因素至少 3 条
- **积分榜与出线（必读）**：
  - 2026 赛制：每组前两名直接出线，12 个小组第三名中成绩最好的 8 支也可晋级（比较积分、净胜球、进球数）
  - \`group_standings\`：本组各队排名、积分、净胜球及出线状态（\`qualification_status_zh\`：已出线/待出线/已无缘出线；\`qualification_note\` 为待出线时的条件说明，含「最佳第三名」争夺）
