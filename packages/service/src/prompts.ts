...
任务：
1. 基于提供的数据（球队、近期状态、历史交锋、比赛信息、上场球员名单与实力评估、积分榜），分析两队的相对优势。
...
3. 给出两组预期进球数：
   ...
   取值范围均 0.3 - 4.0。**评估预期进球时必须把关键球员的实力、状态纳入考虑**。
4. 列出 2-4 名「关键球员」（每队至少 1 名），评估其当前实力（strength 0-10）与状态（form_score 0-10），并在 rationale 中**明确说明依据来源**：
   ...
   - "news"：来自近期新闻/赛事
   - "mixed"：以上综合
5. **为每队的主力 11 人**（从提供的上场球员名单中挑出预期首发，**最多 11 名/队**）给出技术评分：
   - overall / passing / shooting / defense / pace，均为 0-10 浮点
   - rationale：一句话中文短评
   - 必须填 shirt_number（与名单对齐），不要改名/拼写不一致
6. 用中文写一段 3-5 段的研究叙述，其中必须有一段专门讨论「关键球员」与「阵容评分」，另一段分析积分形势对双方心态的影响。
7. 分析积分榜形势：哪些队已出线、哪些需要抢分、哪些已被淘汰，说明这些因素如何影响本场双方的战术选择与比赛动力。

输出必须是严格 JSON，schema 如下：
{
  "expected_goals_home": <float 0.3-4.0>,
  "expected_goals_away": <float 0.3-4.0>,
  "expected_goals_home_upset": <float 0.3-4.0>,
  "expected_goals_away_upset": <float 0.3-4.0>,
  "key_factors": [
  ...
  "squad_ratings": {
    "home": [
      {"player_name": "<与名单完全一致>", "position": "<位置>", "shirt_number": <int|null>, "overall": <float 0-10>, "passing": <float 0-10>, "shooting": <float 0-10>, "defense": <float 0-10>, "pace": <float 0-10>, "rationale": "<中文一句话>"}
    ],
    "away": [ ... 同上 ... ]
  },
  "narrative": "<中文叙述 3-5 段，含关键球员与阵容段、积分形势分析段>",
  "risk_factors": ["<中文风险点>", ...]
}
...
- 阵容评分每队 1-11 人；球员名必须与提供的名单**逐字一致**（含空格、连字符、大小写），否则前端无法对齐
- 各项评分 (overall/passing/shooting/defense/pace) 必须在 [0, 10] 区间
- 叙述要具体引用数据，避免空话
...
- 风险因素至少 3 条
- 积分榜数据非常重要：分析双方的出线形势、抢分动力、已出线/已淘汰状态如何影响本场战术与心态
