...
export function TournamentRankingTable({
  ...
  const expandLabel = collapseLabelCollapsed ?? t("expandAll");
  const collapseLabel = collapseLabelExpanded ?? t("collapseAll");
    ...
    <div className={cn("glass rounded-2xl overflow-hidden", className)}>
      ...
      {collapsible && isCollapsed ? (
        <button
          ...
          className="block w-full px-4 sm:px-5 py-6 text-center text-sm text-white/60 hover:text-white hover:bg-white/[0.02] transition-colors"
        >
          {expandLabel}
        </button>
      ) : (
        <>
            ...
            <table className="w-full min-w-[640px] text-sm">
              <thead>
                <tr className="text-[10px] font-mono uppercase tracking-widest text-white/40 border-b border-white/[0.06]">
                  <th className="px-4 py-3 text-left font-medium w-10">{t("rankCol")}</th>
                  <th className="px-4 py-3 text-left font-medium">{t("teamCol")}</th>
                  {showGroupPosition && (
                    <th className="px-3 py-3 text-center font-medium w-12">{t("groupPosCol")}</th>
                  )}
                  <th className="px-3 py-3 text-center font-medium w-12">{t("playedCol")}</th>
                  <th className="px-3 py-3 text-center font-medium w-12">{t("pointsCol")}</th>
                  <th className="px-3 py-3 text-center font-medium w-12">{t("gdCol")}</th>
                  <th className="px-3 py-3 text-center font-medium w-12">{t("gfCol")}</th>
          ...
          {collapsible && (
            <button
              ...
              className="block w-full px-4 sm:px-5 py-2.5 text-center text-xs text-white/45 hover:text-white/70 border-t border-white/[0.06] transition-colors"
            >
              {collapseLabel}
            </button>
          )}
